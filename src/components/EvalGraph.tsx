...
export function EvalGraph({ points, mistakePlies }: EvalGraphProps) {
  ...
  return (
    <div className="eval-graph">
      <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} role="img" aria-label="形勢グラフ">
      ...
      <p className="hint">上=自分有利 / 下=不利(±{CLAMP_CP}cpで打ち切り)。赤点は検出した悪手・疑問手。</p>
    </div>
  );
}
